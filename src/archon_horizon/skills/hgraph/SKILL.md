...
the warnings rather than trusting `dangling` alone. If the project has **no
blueprint**, `frontier` is empty for the wrong reason: author the chapter
first rather than picking work only from Lean files.
