...
Your invocation prompt contains a line `Slug: <slug>`. Use it for the report filename: `.archon-horizon/reports/refactor-<slug>.md`. Multiple refactors per iteration are allowed and each uses a distinct slug, so do not write to the unsuffixed `reports/refactor.md`.

When you yourself were dispatched from a parent subagent (`ARCHON_SUBAGENT_SLUG` is set), your report lands at `.archon-horizon/reports/<parent-slug>/refactor-<slug>.md` — the Archon CLI handles the path automatically and your invocation prompt names the exact path.
...
- **Document every change** in `reports/refactor-<slug>.md` (see Logging below).
...
9. Write your report to `reports/refactor-<slug>.md`

## Spawning child subagents (optional)

For large refactors that naturally decompose into independent file-level pieces, you may dispatch child subagents via the generic wrapper. To discover what is available this iteration: `ls .archon-horizon/subagents/` and read each descriptor's frontmatter.
...
python3 .claude/tools/archon-subagent.py \
  --name <subagent-name> \
  --slug <child-slug> \
  --directive-file .archon-horizon/logs/iter-NNN/<your-slug>/<name>-<child-slug>-directive.md \
  --write-domain '<glob>' \
  --write-domain '<glob>'
```
...
Write your report to `reports/refactor-<slug>.md` (where `<slug>` is the slug from your invocation prompt). This report is the primary communication channel back to the plan agent — be precise and thorough.
...
| `reports/refactor-<slug>.md` | **write** |
