...
1. Read the target block on disk and inspect it: `horizon dag-query node --node <target> --json` (effort, deps, used-by). `archon` is on PATH; `--json` is parseable (banner on stderr).
...
- **Verify with the graph.** After breaking, run `horizon dag-query node --node <target>` and `horizon dag-query ancestors --node <target>`: the target's effort should drop, the new `L_i` should appear, and there must be no broken `\uses{}`. Fix and re-check.
...
Write your report to `.archon-horizon/reports/effort-breaker-<slug>.md` (or the nested `reports/<parent-slug>/...` path your invocation names).
